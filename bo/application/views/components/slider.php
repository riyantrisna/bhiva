        ...
        <div class="row mb-2">
            <div class="col-sm-12">
                <ol class="breadcrumb float-sm-left">
                    ...
                    <li class="breadcrumb-item active"><?php echo MultiLang('slider'); ?></li>
                </ol>
            </div><!-- /.col -->
        </div><!-- /.row -->
    </div><!-- /.container-fluid -->
	...
	<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<div id="box_msg_slider"></div>
				<form id="form_slider" autocomplete="nope">
...
$(document).ready(function() {
    table = $('#tables').DataTable({
        ...
        "serverSide": true,
        "ajax": {
            "url": "<?php echo base_url(); ?>slider/data",
            "type": "POST"
        },
...
{
    save_method = 'add';
    $('#form_slider')[0].reset(); // reset form on modals
    $("#box_msg_slider").html('').hide();
    $('#btnSave').text('<?php echo MultiLang('save'); ?>');
    $('#btnSave').attr('disabled',false);
    $('#title_form').text('<?php echo MultiLang('add'); ?> <?php echo MultiLang('slider'); ?>'); // Set Title to Bootstrap modal title

    $.ajax({
        url : "<?php echo site_url('slider/add_view/')?>",
        type: "GET",
        dataType: "JSON",
        success: async function(data, textStatus, xhr)
        {
            if(xhr.status == '200'){
                await $('#form_slider').html(data.html);
                //image
...
function edit(id)
{
    ...
    $('#btnSave').text('<?php echo MultiLang('save'); ?>');
    $('#btnSave').attr('disabled',false);
    $('#title_form').text('<?php echo MultiLang('edit'); ?> <?php echo MultiLang('slider'); ?>'); // Set Title to Bootstrap modal title

    $.ajax({
        url : "<?php echo site_url('slider/edit_view/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: async function(data, textStatus, xhr)
        {
...
function save()
{
    ...
    if(save_method == 'add') {
        ...
        url = "<?php echo site_url('slider/edit')?>";
    }
    ...
    $.ajax({
        ...
        success: async function(data, textStatus, xhr)
        {
            if(xhr.status == '200'){
                ...
                {
                    await $('#box_msg_slider').html(data.message).fadeOut().fadeIn();
...
function detail(id)
{
    $('#title_detail').text('<?php echo MultiLang('detail'); ?> <?php echo MultiLang('slider'); ?>'); // Set Title to Bootstrap modal title

    $.ajax({
        url : "<?php echo site_url('slider/detail/')?>/" + id,
        type: "GET",
        dataType: "JSON",
...
function deletes(id,name)
{
    ...
    $('#title_delete').text('<?php echo MultiLang('delete'); ?> <?php echo MultiLang('slider'); ?>'); // Set title to Bootstrap modal title
    $("#body_delete").html('<?php echo MultiLang('delete'); ?> <?php echo MultiLang('slider'); ?> <b>'+name+'</b> ?');
...
function process_delete(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('slider/delete/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data, textStatus, xhr)
        {
