	...
	<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<div id="box_msg_venue"></div>
				<form id="form_venue" autocomplete="nope">
...
$(document).ready(function() {
    table = $('#tables').DataTable({
        ...
        "serverSide": true,
        "ajax": {
            "url": "<?php echo base_url(); ?>venue/data",
            "type": "POST"
        },
...
async function add()
{
    ...
    await $('#form_venue').html('');
    $('#form_venue')[0].reset(); // reset form on modals
    $("#box_msg_venue").html('').hide();
    $('#btnSave').text('<?php echo MultiLang('save'); ?>');
    $('#btnSave').attr('disabled',false);
    $('#title_form').text('<?php echo MultiLang('add'); ?> <?php echo MultiLang('venue'); ?>'); // Set Title to Bootstrap modal title

    $.ajax({
        url : "<?php echo site_url('venue/add_view/')?>",
        type: "GET",
        dataType: "JSON",
        success: async function(data, textStatus, xhr)
        {
            if(xhr.status == '200'){
                ...
                await $('#form_venue').html(data.html);
                //image
...
async function edit(id)
{
    ...
    $('#btnSave').text('<?php echo MultiLang('save'); ?>');
    $('#btnSave').attr('disabled',false);
    $('#title_form').text('<?php echo MultiLang('edit'); ?> <?php echo MultiLang('venue'); ?>'); // Set Title to Bootstrap modal title

    $.ajax({
        url : "<?php echo site_url('venue/edit_view/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: async function(data, textStatus, xhr)
        {
...
async function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('venue/add')?>";
    } else {
        url = "<?php echo site_url('venue/edit')?>";
    }

    $.ajax({
        url : url,
        type: "POST",
        data: $('#form_venue').serialize(),
        dataType: "json",
        success: async function(data, textStatus, xhr)
        {
            if(xhr.status == '200'){
                ...
                {
                    await $('#box_msg_venue').html(data.message).fadeOut().fadeIn();
...
function detail(id)
{
    $('#title_detail').text('<?php echo MultiLang('detail'); ?> <?php echo MultiLang('venue'); ?>'); // Set Title to Bootstrap modal title

    $.ajax({
        url : "<?php echo site_url('venue/detail/')?>/" + id,
        type: "GET",
        dataType: "JSON",
...
function deletes(id,name)
{
    ...
    $('#title_delete').text('<?php echo MultiLang('delete'); ?> <?php echo MultiLang('venue'); ?>'); // Set title to Bootstrap modal title
    $("#body_delete").html('<?php echo MultiLang('delete'); ?> <?php echo MultiLang('venue'); ?> <b>'+name+'</b> ?');
...
function process_delete(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('venue/delete/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data, textStatus, xhr)
        {
