        ...
        <div class="row mb-2">
            <div class="col-sm-12">
                <ol class="breadcrumb float-sm-left">
                    ...
                    <li class="breadcrumb-item active"><?php echo MultiLang('travelpost'); ?></li>
                </ol>
            </div><!-- /.col -->
        </div><!-- /.row -->
    </div><!-- /.container-fluid -->
	...
	<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<div id="box_msg_travelpost"></div>
				<form id="form_travelpost" autocomplete="nope">
...
$(document).ready(function() {
    table = $('#tables').DataTable({
        ...
        "serverSide": true,
        "ajax": {
            "url": "<?php echo base_url(); ?>travelpost/data",
            "type": "POST"
        },
...
async function add()
{
    ...
    await $('#form_travelpost').html('');
    $('#form_travelpost')[0].reset(); // reset form on modals
    $("#box_msg_travelpost").html('').hide();
    $('#btnSave').text('<?php echo MultiLang('save'); ?>');
    $('#btnSave').attr('disabled',false);
    $('#title_form').text('<?php echo MultiLang('add'); ?> <?php echo MultiLang('travelpost'); ?>'); // Set Title to Bootstrap modal title

    $.ajax({
        url : "<?php echo site_url('travelpost/add_view/')?>",
        type: "GET",
        dataType: "JSON",
        success: async function(data, textStatus, xhr)
        {
            if(xhr.status == '200'){
                ...
                await $('#form_travelpost').html(data.html);
                //image
...
async function edit(id)
{
    ...
    $('#btnSave').text('<?php echo MultiLang('save'); ?>');
    $('#btnSave').attr('disabled',false);
    $('#title_form').text('<?php echo MultiLang('edit'); ?> <?php echo MultiLang('travelpost'); ?>'); // Set Title to Bootstrap modal title

    $.ajax({
        url : "<?php echo site_url('travelpost/edit_view/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: async function(data, textStatus, xhr)
        {
...
async function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('travelpost/add')?>";
    } else {
        url = "<?php echo site_url('travelpost/edit')?>";
    }

    $.ajax({
        url : url,
        type: "POST",
        data: $('#form_travelpost').serialize(),
        dataType: "json",
        success: async function(data, textStatus, xhr)
        {
            if(xhr.status == '200'){
                ...
                {
                    await $('#box_msg_travelpost').html(data.message).fadeOut().fadeIn();
...
function detail(id)
{
    $('#title_detail').text('<?php echo MultiLang('detail'); ?> <?php echo MultiLang('travelpost'); ?>'); // Set Title to Bootstrap modal title

    $.ajax({
        url : "<?php echo site_url('travelpost/detail/')?>/" + id,
        type: "GET",
        dataType: "JSON",
...
function deletes(id,name)
{
    ...
    $('#title_delete').text('<?php echo MultiLang('delete'); ?> <?php echo MultiLang('travelpost'); ?>'); // Set title to Bootstrap modal title
    $("#body_delete").html('<?php echo MultiLang('delete'); ?> <?php echo MultiLang('travelpost'); ?> <b>'+name+'</b> ?');
...
function process_delete(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('travelpost/delete/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data, textStatus, xhr)
        {
