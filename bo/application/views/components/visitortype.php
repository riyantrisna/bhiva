        ...
        <div class="row mb-2">
            <div class="col-sm-12">
                <ol class="breadcrumb float-sm-left">
                    ...
                    <li class="breadcrumb-item active"><?php echo MultiLang('visitor_type'); ?></li>
                </ol>
            </div><!-- /.col -->
        </div><!-- /.row -->
    </div><!-- /.container-fluid -->
	...
	<div class="modal-dialog modal-dialog-centered" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<div id="box_msg_visitortype"></div>
				<form id="form_visitortype">
...
$(document).ready(function() {
    table = $('#tables').DataTable({
        ...
        "serverSide": true,
        "ajax": {
            "url": "<?php echo base_url(); ?>visitortype/data",
            "type": "POST"
        },
...
{
    save_method = 'add';
    $('#form_visitortype')[0].reset(); // reset form on modals
    $("#box_msg_visitortype").html('').hide();
    $('#btnSave').text('<?php echo MultiLang('save'); ?>');
    $('#btnSave').attr('disabled',false);
    $('#title_form').text('<?php echo MultiLang('add'); ?> <?php echo MultiLang('visitor_type'); ?>'); // Set Title to Bootstrap modal title

    $.ajax({
        url : "<?php echo site_url('visitortype/add_view/')?>",
        type: "GET",
        dataType: "JSON",
        success: function(data, textStatus, xhr)
        {
            if(xhr.status == '200'){
                $('#form_visitortype').html(data.html);
            }else{
...
function edit(id)
{
    ...
    $('#btnSave').text('<?php echo MultiLang('save'); ?>');
    $('#btnSave').attr('disabled',false);
    $('#title_form').text('<?php echo MultiLang('edit'); ?> <?php echo MultiLang('visitor_type'); ?>'); // Set Title to Bootstrap modal title

    $.ajax({
        url : "<?php echo site_url('visitortype/edit_view/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data, textStatus, xhr)
        {
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('visitortype/add')?>";
    } else {
        url = "<?php echo site_url('visitortype/edit')?>";
    }

    $.ajax({
        url : url,
        type: "POST",
        data: $('#form_visitortype').serialize(),
        dataType: "json",
        success: async function(data, textStatus, xhr)
        {
            if(xhr.status == '200'){
                ...
                {
                    $('#box_msg_visitortype').html(data.message).fadeOut().fadeIn();
                }
            }else{
                $('#modal_form').modal('toggle');
                toastr.error(xhr.statusText);
...
function detail(id)
{
    $('#title_detail').text('<?php echo MultiLang('detail'); ?> <?php echo MultiLang('visitor_type'); ?>'); // Set Title to Bootstrap modal title

    $.ajax({
        url : "<?php echo site_url('visitortype/detail/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data, textStatus, xhr)
        {
...
function deletes(id,name)
{
    ...
    $('#title_delete').text('<?php echo MultiLang('delete'); ?> <?php echo MultiLang('visitortype'); ?>'); // Set title to Bootstrap modal title
    $("#body_delete").html('<?php echo MultiLang('delete'); ?> <?php echo MultiLang('visitortype'); ?> <b>'+name+'</b> ?');
...
function process_delete(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('visitortype/delete/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data, textStatus, xhr)
        {
