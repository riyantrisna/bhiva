        ...
        <div class="row mb-2">
            <div class="col-sm-12">
                <ol class="breadcrumb float-sm-left">
                    ...
                    <li class="breadcrumb-item active"><?php echo MultiLang('user'); ?></li>
                </ol>
            </div><!-- /.col -->
        </div><!-- /.row -->
    </div><!-- /.container-fluid -->
                    ...
                    <table id="tables" class="table table-bordered table-hover dt-responsive nowrap" style="width:100%">
                        <thead>
                            <tr>
                                ...
                                <th><?php echo MultiLang('email'); ?></th>
                                <th><?php echo MultiLang('phone'); ?></th>
                                <th><?php echo MultiLang('admin'); ?>?</th>
...
function readURL(input, selector) {
    if (input.files && input.files[0]) {
        ...
        $('#file_photo_show').fadeOut().fadeIn();
        $('#remove_file').show();
        $('#selector_file').hide();
    }
}
...
function add()
{
    ...
    $('#title_form').text('<?php echo MultiLang('add'); ?> <?php echo MultiLang('user'); ?>'); // Set Title to Bootstrap modal title

    $.ajax({
        url : "<?php echo site_url('user/add_view/')?>",
        type: "GET",
        dataType: "JSON",
...
function edit(id)
{
    ...
    $('#btnSave').text('<?php echo MultiLang('save'); ?>');
    $('#btnSave').attr('disabled',false);
    $('#title_form').text('<?php echo MultiLang('edit'); ?> <?php echo MultiLang('user'); ?>'); // Set Title to Bootstrap modal title

    $.ajax({
        url : "<?php echo site_url('user/edit_view/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: async function(data, textStatus, xhr)
        {
...
function detail(id)
{
    $('#title_detail').text('<?php echo MultiLang('detail'); ?> <?php echo MultiLang('user'); ?>'); // Set Title to Bootstrap modal title

    $.ajax({
        url : "<?php echo site_url('user/detail/')?>/" + id,
        type: "GET",
        dataType: "JSON",
...
function deletes(id,name)
{
    ...
    $('#title_delete').text('<?php echo MultiLang('delete'); ?> <?php echo MultiLang('user'); ?>'); // Set title to Bootstrap modal title
    $("#body_delete").html('<?php echo MultiLang('delete'); ?> <?php echo MultiLang('user'); ?> <b>'+name+'</b> ?');
