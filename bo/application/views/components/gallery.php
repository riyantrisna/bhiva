        ...
        <div class="row mb-2">
            <div class="col-sm-12">
                <ol class="breadcrumb float-sm-left">
                    ...
                    <li class="breadcrumb-item active"><?php echo MultiLang('gallery'); ?></li>
                </ol>
            </div><!-- /.col -->
        </div><!-- /.row -->
    </div><!-- /.container-fluid -->
	...
	<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<div id="box_msg_gallery"></div>
				<form id="form_gallery" autocomplete="nope">
...
function togalleryimages(id)
{
    $.ajax({
        url : "<?php echo base_url(); ?>gallery/setid",
        type: "POST",
        data: {"id": id},
        dataType: "json",
        success: async function(data, textStatus, xhr)
        {
            if(xhr.status == '200'){
                if(data.status)
                { 
                    window.location.href = "<?php echo base_url(); ?>galleryimages";
                }
                else
                {
...
{
    save_method = 'add';
    $('#form_gallery')[0].reset(); // reset form on modals
    $("#box_msg_gallery").html('').hide();
    $('#btnSave').text('<?php echo MultiLang('save'); ?>');
    $('#btnSave').attr('disabled',false);
    $('#title_form').text('<?php echo MultiLang('add'); ?> <?php echo MultiLang('gallery'); ?>'); // Set Title to Bootstrap modal title

    $.ajax({
        url : "<?php echo site_url('gallery/add_view/')?>",
        type: "GET",
        dataType: "JSON",
        success: async function(data, textStatus, xhr)
        {
            if(xhr.status == '200'){
                await $('#form_gallery').html(data.html);
                //image
...
function edit(id)
{
    ...
    $('#btnSave').text('<?php echo MultiLang('save'); ?>');
    $('#btnSave').attr('disabled',false);
    $('#title_form').text('<?php echo MultiLang('edit'); ?> <?php echo MultiLang('gallery'); ?>'); // Set Title to Bootstrap modal title

    $.ajax({
        url : "<?php echo site_url('gallery/edit_view/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: async function(data, textStatus, xhr)
        {
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('gallery/add')?>";
    } else {
        url = "<?php echo site_url('gallery/edit')?>";
    }

    $.ajax({
        url : url,
        type: "POST",
        data: $('#form_gallery').serialize(),
        dataType: "json",
        success: async function(data, textStatus, xhr)
        {
            if(xhr.status == '200'){
                ...
                {
                    await $('#box_msg_gallery').html(data.message).fadeOut().fadeIn();
...
function detail(id)
{
    $('#title_detail').text('<?php echo MultiLang('detail'); ?> <?php echo MultiLang('gallery'); ?>'); // Set Title to Bootstrap modal title

    $.ajax({
        url : "<?php echo site_url('gallery/detail/')?>/" + id,
        type: "GET",
        dataType: "JSON",
...
function deletes(id,name)
{
    ...
    $('#title_delete').text('<?php echo MultiLang('delete'); ?> <?php echo MultiLang('gallery'); ?>'); // Set title to Bootstrap modal title
    $("#body_delete").html('<?php echo MultiLang('delete'); ?> <?php echo MultiLang('gallery'); ?> <b>'+name+'</b> ?');
