        ...
        <div class="row mb-2">
            <div class="col-sm-12">
                <ol class="breadcrumb float-sm-left">
                    ...
                    <li class="breadcrumb-item"><a href="<?php echo base_url(); ?>gallery"><?php echo MultiLang('gallery'); ?></a></li>
                    <li class="breadcrumb-item active"><?php echo MultiLang('galleryimages').' <b>('.$title.')</b>'; ?></li>
                </ol>
            </div><!-- /.col -->
        </div><!-- /.row -->
    </div><!-- /.container-fluid -->
                    ...
                    <table id="tables" class="table table-bordered table-hover dt-responsive nowrap" style="width:100%">
                        <thead>
                            <tr>
                                ...
                                <th><?php echo MultiLang('title'); ?></th>
	...
	<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<div id="box_msg_galleryimages"></div>
				<form id="form_galleryimages" autocomplete="nope">
...
$(document).ready(function() {
    table = $('#tables').DataTable({
        ...
        "serverSide": true,
        "ajax": {
            "url": "<?php echo base_url(); ?>galleryimages/data/<?php echo $id;?>",
            "type": "POST"
        },
...
{
    save_method = 'add';
    $('#form_galleryimages')[0].reset(); // reset form on modals
    $("#box_msg_galleryimages").html('').hide();
    $('#btnSave').text('<?php echo MultiLang('save'); ?>');
    $('#btnSave').attr('disabled',false);
    $('#title_form').text('<?php echo MultiLang('add'); ?> <?php echo MultiLang('galleryimages'); ?>'); // Set Title to Bootstrap modal title

    $.ajax({
        url : "<?php echo site_url('galleryimages/add_view/')?>",
        type: "GET",
        dataType: "JSON",
        success: async function(data, textStatus, xhr)
        {
            if(xhr.status == '200'){
                await $('#form_galleryimages').html(data.html);
                //image
...
function edit(id)
{
    ...
    $('#btnSave').text('<?php echo MultiLang('save'); ?>');
    $('#btnSave').attr('disabled',false);
    $('#title_form').text('<?php echo MultiLang('edit'); ?> <?php echo MultiLang('galleryimages'); ?>'); // Set Title to Bootstrap modal title

    $.ajax({
        url : "<?php echo site_url('galleryimages/edit_view/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: async function(data, textStatus, xhr)
        {
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('galleryimages/add')?>";
    } else {
        url = "<?php echo site_url('galleryimages/edit')?>";
    }

    $.ajax({
        url : url,
        type: "POST",
        data: $('#form_galleryimages').serialize(),
        dataType: "json",
        success: async function(data, textStatus, xhr)
        {
            if(xhr.status == '200'){
                ...
                {
                    await $('#box_msg_galleryimages').html(data.message).fadeOut().fadeIn();
...
function detail(id)
{
    $('#title_detail').text('<?php echo MultiLang('detail'); ?> <?php echo MultiLang('galleryimages'); ?>'); // Set Title to Bootstrap modal title

    $.ajax({
        url : "<?php echo site_url('galleryimages/detail/')?>/" + id,
        type: "GET",
        dataType: "JSON",
...
function deletes(id,name)
{
    ...
    $('#title_delete').text('<?php echo MultiLang('delete'); ?> <?php echo MultiLang('galleryimages'); ?>'); // Set title to Bootstrap modal title
    $("#body_delete").html('<?php echo MultiLang('delete'); ?> <?php echo MultiLang('galleryimages'); ?> <b>'+name+'</b> ?');
...
function process_delete(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('galleryimages/delete/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data, textStatus, xhr)
        {
