...
insert  into `cms_contact`(`contact_id`,`contact_address`,`contact_email`,`contact_phone`,`contact_wa`,`contact_fb`,`contact_ig`,`contact_twitter`,`contact_img_maps`,`contact_link_maps`,`insert_user_id`,`insert_datetime`,`update_user_id`,`update_datetime`) values (1,'<p><span xss=removed>Jl. Ringroad Utara 66, Nayan, Nayan, Maguwoharjo, Kec. Depok, Kabupaten Sleman, Daerah Istimewa Yogyakarta 55281</span><br></p>','support@bhiva.id','0274484273','085787654654','rrr','rr','rr','c07feed49043e8cc4048b9f204905f88.png','https://www.google.com/maps/place/Bhiva+Tour+&+Travel/@-7.7784834,110.4301161,15z/data=!4m5!3m4!1s0x2e7a5a1f1946139f:0x15c4a0a33aefd5f!8m2!3d-7.77446!4d110.430379?hl=id',1,'2019-09-20 18:11:59',5,'2019-11-01 07:09:09');
...
insert  into `mst_destination`(`destination_id`,`destination_desloc_id`,`destination_status`,`insert_user_id`,`insert_datetime`,`update_user_id`,`update_datetime`) values (1,1,1,5,'2019-09-22 23:37:16',5,'2019-11-03 21:00:50');
...
insert  into `mst_destination_img`(`destinationimg_destination_id`,`destinationimg_order`,`destinationimg_img`) values (1,1,'a489549be3a20c1810987997a57a7d09.jpeg'),(1,2,NULL),(1,3,'5ef8e1e904dfbc4962eca54b16f621ea.png'),(1,4,NULL);
...
CREATE TABLE `trx_transaction` (
  `1` bigint(20) NOT NULL AUTO_INCREMENT,
...
insert  into `trx_transaction`(`1`,`transaction_code`,`transaction_date`,`transaction_type`,`transaction_user_id`,`transaction_user_real_name`,`transaction_total`,`transaction_midtrans_transaction_id`,`transaction_midtrans_response`,`transaction_status`,`transaction_desc`,`insert_user_id`,`insert_datetime`,`update_user_id`,`update_datetime`) values (1,NULL,NULL,1,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL);
...
CREATE TABLE `trx_transaction_tourpackages` (
  ...
  CONSTRAINT `trx_transaction_tourpackages_ibfk_1` FOREIGN KEY (`transactiontourpackages_transaction_id`) REFERENCES `trx_transaction` (`1`) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE `trx_transaction_venue` (
  ...
  CONSTRAINT `trx_transaction_venue_ibfk_1` FOREIGN KEY (`transactionvenue_transaction_id`) REFERENCES `trx_transaction` (`1`) ON DELETE CASCADE ON UPDATE CASCADE,
